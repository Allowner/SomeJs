...
app.get("/news/:source/skip/:number", async function (req, res) {
  ...
  var result;
  if (paramSource !== "me") {
    const response = await fetch(`https://newsapi.org/v2/everything?sources=${paramSource}&apiKey=f7f0c071d73c4186ad64b9fdbd0fb6f0`);
    if (!response.ok) {
      throw new Error('Network response was not ok.');
    }
      ...
      result.articles.forEach(element => {
        News.findOne({ url: { $eq: element.url } }, function (err, item) {
          ...
          if (item == null) {
            ...
            lastId += 1;
            console.log(lastId, "fff!");
            newsObject.save();
          }
        });
      });
      console.log(lastId, "!");
      LastId.updateOne({}, { $set: { lastId: lastId } }, { upsert: true }, function (err, result) { });
    });
  }
  News.find({ "source.name": { $eq: paramSource }, skip: paramSkip, limit: 5 }, function (err, news) {
    if (err) {
      throw new Error(err.message);
    }
